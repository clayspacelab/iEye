function [ii_data,ii_cfg,ii_sacc,ii_reach] = ii_preproc_mgrs(edf_fn,cfg_fn,preproc_fn,ii_params,trialinfo,skip_steps,mgrs_header_fn,mgrs_noheader_fn)
...
if nargin < 7 
    mgrs_header_fn =[];
    disp('proceeding without mocap .. dont!')
end


if nargin < 8 
    mgrs_header_fn =[];
    disp('forget the reach data then')
end
...
% compute velocity on reach
[ii_data,ii_cfg] = ii_reach_velocity(ii_data,ii_cfg,'rX','rY'); %change to reach_velocity 

[ii_data] = ii_checkfor_reaches(ii_data); 
...
% LOOK FOR REACHES : NOTE thresholds have not been changed for reaches from
% saccades
...
[ii_data,ii_cfg] = ii_findreachfixations(ii_data,ii_cfg,{'rX','rY'},ii_params.fixation_mode);
...
if ~ismember('drift',skip_steps)
    ...
    [ii_data,ii_cfg] = ii_selectreachfixationsbytrial( ii_data, ii_cfg, ii_params.epoch_chan,...
        ii_params.drift_epoch, ii_params.drift_fixation_mode );
...
% 'raw' or 'smoothed' data on each channel)
    ...
    [ii_data,ii_cfg] = ii_reach_driftcorrect(ii_data,ii_cfg,{'rX','rY'},ii_params.drift_select_mode,...
        ii_params.drift_target);
...
if ~ismember('calibration',skip_steps)
    ...
    %added 6.25.19 geh
     [ii_data,ii_cfg] = ii_selectreachfixationsbytrial(ii_data,ii_cfg,ii_params.epoch_chan,...
        ii_params.calibrate_epoch,ii_params.calibrate_select_mode,ii_params.calibrate_window,...
        ii_params.calibrate_target,{'rX_fix','rY_fix'}); % 
    ...
    if ismember(ii_params.calibrate_mode,{'scale','rotate'})
        ...
        [ii_data,ii_cfg] = ii_reach_calibratebytrial(ii_data,ii_cfg,{'rX','rY'},...
            ii_params.calibrate_target,ii_params.calibrate_mode, ii_params.calibrate_limits);
       
        
    elseif ismember(ii_params.calibrate_mode,'run')
        % calibrate by run / dont need to do this for reaches 
...
f_all = ii_plotalltrials(ii_data,ii_cfg,{'X','Y'},[],[],ii_params.epoch_chan,ii_params.plot_epoch,1);

r_all = ii_plotalltrials_reach(ii_data,ii_cfg,{'rX','rY'},[],[],ii_params.epoch_chan,ii_params.plot_epoch,1)
...
% get the saccades %do this separately for reach, implement new ii_reach
% struct
...
[ii_data,ii_cfg,ii_reach] = ii_extractreaches(ii_data,ii_cfg,{'rX','rY'},...
    ii_params.extract_sacc_mode_start,...
    ii_params.extract_sacc_mode_end,...
    ii_params.epoch_chan);
%else
...
ii_savereach(ii_cfg,ii_reach,[mgrs_header_fn(1:end-4) '_reach.mat']);
