function [ii_data,ii_cfg] = ii_import_edf_mgrs(tsv_file,ifg_file,data_file, varargin)
...
if nargin < 1 || isempty(tsv_file)
    [filename, pathname] = uigetfile('*.tsv', 'Select tsv file');
...
if nargin < 3
   ...
   data_file = sprintf('%s_iEye.mat',tsv_file(1:(end-4)));
    
end

if isempty(data_file)
    iEye_file = sprintf('%s_iEye.mat',tsv_file(1:(end-4))); %strrep(edf_file, '.edf', '.mat');
...
%else
[status,result] = system(['edf2asc -t -c -s -miss 0 ' tsv_file]);
%end
disp(status);
disp(result);
asc_samp_file = strrep(tsv_file, '.tsv', '.asc');
...
disp(status);
disp(result);
asc_evnt_file = strrep(tsv_file, '.tsv', '.asc');
...
%mgrs: find target.angle replace with xDAT code (14 = saccade; 15 =
%corrective)
find_sacc_vals =[30 60 120 150 210 240 300 330]; 
find_corr_vals = find_sacc_vals+5;

sacc_ind = ismember(vval,find_sacc_vals);
vval(find(sacc_ind)) = 14;
vval((find(sacc_ind))+1) = 15;
%here, get ready of wrongly assigned tarX,tarY outputs to XDAT during
%target epoch and set to 0 
xdat12 = [12];
xdat12_ind = find(ismember(vval,xdat12));
xdat13 = [13];
xdat13_ind = xdat12_ind +1;
for ii =1:length(xdat12_ind)
if vval(xdat13_ind(ii)) ~=13
    vval(xdat13_ind(ii)) = 0;
     vval(xdat13_ind(ii)+1) = 0;
else
end 
end 
