...
%
% Usage:
%   ii_plotQC_exclusions(ii_trial_reach,ii_cfg) plots n_runs (if r_num present;
...
%
%   fh = ii_plotQC_exclusions(ii_trial_reach,ii_cfg,...) returns figure handles
%   for all plots
%
%   ii_plotQC_exclusions(ii_trial_reach,ii_cfg,which_excl) allows specification
%   of which types of exclusion criteria to respect (by default, all)
%
%   ii_plotQC_exclusions(ii_trial_reach,ii_cfg,[],fig_visible), where fig_visible
%   is a logical, determines whether the plotted figures are visible, or if
%   they're plotted in the background
%
% ii_plotQC functions are compatible with ii_trial_reach variables, computed with
% ii_scoreMGS.m, or ii_sess variables built from concatenated ii_trial_reach
...
if isfield(ii_trial_reach,'r_num') && length(unique(ii_trial_reach.r_num))>1 % if we need multiple subplots...
    ru_dot = unique(ii_trial_reach.r_num);
    this_ncols = min(length(ru_dot),MAX_COLS);
    this_nrows = min(ceil(length(ru_dot)/MAX_COLS),MAX_ROWS);
    nfigs_dot = ceil(length(unique(ii_trial_reach.r_num))/(MAX_ROWS*MAX_COLS)); % hopefully just one...
else
    this_nrows = 1;
    this_ncols = 1;
    ru_dot = 1;
...
for rr = 1:length(ru_dot)
    ...
    if isfield(ii_trial_reach,'r_num')
        thisidx = find(ii_trial_reach.r_num==ru_dot(rr));
    else
        thisidx = find(ones(size(ii_trial_reach.i_sacc,1),1)==1);
    end
    ...
    for tt = 1:length(thisidx)
        ...
        if ~isempty(ii_trial_reach.excl_trial{thisidx(tt)})
            ...
            this_excl = ii_trial_reach.excl_trial{thisidx(tt)};
...
if isfield(ii_trial_reach,'r_num') && length(unique(ii_trial_reach.r_num))>1 % there are runs, plot evolution thru time
    this_nrows = 2;
    this_ncols = 2;
...
for ee = 1:length(all_excl)
    ...
    this_excl = cellfun(@any,cellfun(@(a) ismember(a,all_excl(ee)),ii_trial_reach.excl_trial,'UniformOutput',false));
...
this_excl = ~cellfun(@isempty,ii_trial_reach.excl_trial);
bar(1,100*mean(this_excl),'EdgeColor',[0 0 0],'FaceColor',[0 0 0],'LineWidth',1.5);
clear this_excl;

this_excl = cellfun(@any,cellfun(@(a) ismember(a,which_excl),ii_trial_reach.excl_trial,'UniformOutput',false));
...
if isfield(ii_trial_reach,'r_num')
    ...
    for rr = 1:length(ru)
        ...
        this_excl_all   = ~cellfun(@isempty,{ii_trial_reach.excl_trial{ii_trial_reach.r_num==ru(rr)}});
        this_excl_which = cellfun(@any,cellfun(@(a) ismember(a,which_excl),{ii_trial_reach.excl_trial{ii_trial_reach.r_num==ru(rr)}},'UniformOutput',false));
