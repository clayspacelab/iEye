...
if ~isempty(strfind(ii_cfg.reach_file,'/'))
    fn_fortitle = ii_cfg.reach_file(strfind(ii_cfg.reach_file,'/')+1:end);
else
    fn_fortitle = ii_cfg.reach_file;
end
...
while trial_cnt <= ii_cfg.numtrials
    ...
    ii_plottrial(ii_data,ii_cfg,trial_cnt,{'rX','rY'},thisax,'condensed',epoch_chan,epoch_nums);
