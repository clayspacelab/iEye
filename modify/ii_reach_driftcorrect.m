function [ ii_data, ii_cfg ] = ii_reach_driftcorrect( ii_data, ii_cfg, chan_names, correct_mode, target_coords )
...
numsel = size(ii_cfg.reach_cursel,1);
...
for cc = 1:length(chan_names)
    ...
    for sel_idx = 1:size(ii_cfg.reach_cursel,1)
        
        % look up trial # at start of selection (this should be
        % fine...)
        which_trial = ii_cfg.trialvec(ii_cfg.reach_cursel(sel_idx,1));
        ...
        if strcmpi(correct_mode,'fixation')
            adj_by = nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(ii_cfg.reach_cursel(sel_idx,1):ii_cfg.reach_cursel(sel_idx,2)))-target_coords(cc);
        elseif strcmpi(correct_mode,'smooth')
            adj_by = nanmean(ii_data.(sprintf('%s_smooth',chan_names{cc}))(ii_cfg.reach_cursel(sel_idx,1):ii_cfg.reach_cursel(sel_idx,2)))-target_coords(cc);
        else
            adj_by = nanmean(ii_data.(chan_names{cc})(ii_cfg.reach_cursel(sel_idx,1):ii_cfg.reach_cursel(sel_idx,2)))-target_coords(cc);
        end
        ...
        ii_cfg.reach_drift.amt(which_trial,cc) = adj_by;
...
ii_cfg.reach_history{end+1} = sprintf('ii_driftcorrect - chans %s, mode %s, to target coords [%s] - %s',chan_str,correct_mode,num2str(target_coords), datestr(now,30)  );
