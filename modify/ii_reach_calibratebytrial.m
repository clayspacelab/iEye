function [ ii_data, ii_cfg ] = ii_reach_calibratebytrial( ii_data, ii_cfg, chan_names, calib_targets, calib_mode, adj_limit )
    ...
    if sum(cellfun(@ischar,calib_targets))==length(calib_targets)
        ...
        for cc = 1:length(calib_targets)
            for tt = 1:length(tu)
                ...
                calibrate_to(tt,cc) = mean(ii_data.(calib_targets{cc})(ii_cfg.reach_sel&tr_idx));
...
ii_cfg.reach_calibrate.fcn = 'ii_reach_calibratebytrial.m';
ii_cfg.reach_calibrate.chan = chan_names; % channels input to ii_driftcorrect
ii_cfg.reach_calibrate.mode = calib_mode;
ii_cfg.reach_calibrate.amt = nan(ii_cfg.numtrials,length(chan_names));
ii_cfg.reach_calibrate.err = nan(ii_cfg.numtrials,1);
ii_cfg.reach_calibrate.adj_limits = adj_limit;
ii_cfg.reach_calibrate.adj = zeros(ii_cfg.numtrials,1)==1; % whether adjustment was performed on this trial

% add indicator(s) for why a trial wasn't adjusted
ii_cfg.reach_calibrate.excl_info = cell(ii_cfg.numtrials,1); % fill this in with information about why a trial was excluded
% codes:
...
if strcmpi(calib_mode,'scale')
    ...
    for tt = 1:length(tu)
        ...
        for cc = 1:length(chan_names)
            ...
            if sum(ii_cfg.reach_sel & tr_idx) > 0
                ...
                adj_by = nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(tr_idx & ii_cfg.reach_sel) )/calibrate_to(tt,cc);
                
                % difference fix - targ
                this_err(cc) = nanmean(ii_data.(sprintf('%s_fix',chan_names{cc}))(tr_idx & ii_cfg.reach_sel) ) - calibrate_to(tt,cc); 
                
            else
                
                warning('iEye:ii_calibratebytrial:noSelectedTimepoints','Trial %i: no selected timepoints; forgoing calibration\n',tu(tt));
                adj_by = NaN;
                ii_cfg.reach_calibrate.excl_info{tt}(end+1) = 1;
            end
            
            % save the ratio used for calibration
            % (NOTE: still save, even if we don't actually apply this!!!)
            ii_cfg.reach_calibrate.amt(tt,cc) = adj_by;
            
            
            clear adj_by;
        end
        
        ii_cfg.reach_calibrate.err(tt) = sqrt(sum(this_err.^2));
        clear this_err;
    ...
    for cc = 1:length(chan_names)
        ...
        for tt = 1:length(tu)
            ...
            tr_idx = ii_cfg.trialvec==tu(tt);
                ...
                if ii_cfg.reach_calibrate.err(tt) <= adj_limit && ii_cfg.reach_calibrate.err(tt) > eps % only calibrate if within distnace, but > 0 
                ...
                % within spec (ii_cfg.calibrate.err(tt) <= adj_limits)
                    for ca = 1:length(chans_to_adjust)
                        ...
                        ii_data.(chans_to_adjust{ca})(tr_idx)=ii_data.(chans_to_adjust{ca})(tr_idx)./ii_cfg.reach_calibrate.amt(tt,cc);
                    end
                    
                    ii_cfg.reach_calibrate.adj(tt) = 1==1;
                   
                else
                    if ii_cfg.reach_calibrate.err(tt) >= adj_limit
                        ii_cfg.reach_calibrate.excl_info{tt}(end+1)=2;
                    else
                        ii_cfg.reach_calibrate.excl_info{tt}(end+1)=3; % no adj after feedback
                    end
                end
