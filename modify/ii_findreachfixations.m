function [ii_data,ii_cfg] = ii_findreachfixations(ii_data,ii_cfg,fix_chans,fix_method)
...
% check whether ii_cfg.reaches
assert(isfield(ii_cfg,'reaches')); 
%assert(isfield(ii_cfg,'blinks')); %this is not necessary for reaches...
%obviously!
...
[ii_data,ii_cfg] = ii_setreachselect(ii_data,ii_cfg,'reaches'); % set sel to ii_cfg.('saccades');
[ii_data,ii_cfg] = ii_selectreachinverse(ii_data,ii_cfg);
...
for cc = 1:length(fix_chans)
    ...
    for ff = 1:size(ii_cfg.reach_cursel,1)
        
        fix_idx = ii_cfg.reach_cursel(ff,1):ii_cfg.reach_cursel(ff,2);
...
ii_cfg.reach_history{end+1} = sprintf('ii_findreachfixations - created channels %s, method %s - %s',new_chan_str,fix_method,datestr(now,30));
