function [ii_sess_reach] = ii_combineruns_reach(ii_trial,run_labels)
...
r_num = nan(size(ii_trial{1}.i_reach,1)*length(ii_trial),1);
t_num = nan(size(ii_trial{1}.i_reach,1)*length(ii_trial),1);

for rr = 1:length(ii_trial)
    ii_sess_reach = cat_struct(ii_sess_reach,ii_trial{rr},{'params'}); % don't need to concatenate params
    
    thisidx = size(ii_sess_reach.i_reach,1) - size(ii_trial{rr}.i_reach,1) + (1:size(ii_trial{rr}.i_reach,1));
    
    r_num(thisidx) = run_labels(rr);
    t_num(thisidx) = 1:size(ii_trial{rr}.i_reach,1);
    clear thisidx;
end


ii_sess_reach.params = ii_trial{1}.params;
