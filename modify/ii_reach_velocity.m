function [ii_data,ii_cfg] = ii_reach_velocity(ii_data,ii_cfg,xchan,ychan)
...
if nargin == 2
    prompt = {'Channel 1 (rX)', 'Channel 2 (rY)'};
    dlg_title = 'Velocity';
    num_lines = 1;
...
if ismember(xchan,fieldnames(ii_data)) && ismember(ychan,fieldnames(ii_data))
    ...
    ii_cfg.reach_velocity = [0; sqrt(diff(ii_data.(xchan)).^2 + diff(ii_data.(ychan)).^2) ] * ii_cfg.hz;
    
    ii_cfg.reach_history{end+1} = sprintf('ii_reach_velocity (%s,%s) - %s ', xchan, ychan, datestr(now,30));
    
    % added 6/13/2018 - save velocity as a channel in ii_data
    ii_data.reach_velocity = ii_cfg.reach_velocity;
