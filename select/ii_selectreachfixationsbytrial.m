function [ ii_data, ii_cfg ] = ii_selectreachfixationsbytrial( ii_data, ii_cfg, epoch_chan, within_epochs, sel_mode, buffer_window,calibrate_targs,fix_channels )
...
if nargin < 8
    fix_channels = {'rX_fix','rY_fix'}; % where to use for alignment to calibration targets on 'nearest' mode
end
...
if ~ismember(fieldnames(ii_cfg),'reach_fixations')
    error('iEye:ii_selectfixationsbytrial:missingFixations', 'Fixations not computed - run ii_findreachfixations prior to selecting fixations');
end
...
if any(strcmpi(sel_mode,{'all','mode'}))
    all_fixvec = 0*ii_cfg.reach_sel;
    for ff = 1:size(ii_cfg.reach_fixations)
        all_fixvec(ii_cfg.reach_fixations(ff,1):ii_cfg.reach_fixations(ff,2))=1;
    end
end
...
for tt = 1:length(tu)
    ...
    if strcmpi(sel_mode,'last') % used for drift correction, calibration
        ...
        last_fix_ind = find(ii_cfg.reach_fixations(:,1)<(epoch_end-buffer_window),1,'last');
        
        last_fix_vec = zeros(size(new_sel));
        last_fix_vec(max(ii_cfg.reach_fixations(last_fix_ind,1),epoch_begin):(epoch_end-buffer_window))=1;
    ...
    elseif strcmpi(sel_mode,'first')
        ...
        first_fix_ind = find(ii_cfg.reach_fixations(:,1)>epoch_begin,1,'first');
        ...
        first_fix_vec(ii_cfg.reach_fixations(first_fix_ind,1):min(epoch_end,ii_cfg.reach_fixations(first_fix_ind,2)))=1;
    ...
    elseif strcmpi(sel_mode,'begin')
        ...
        first_fix_vec = zeros(size(new_sel));
        first_fix_vec(epoch_begin:min(ii_cfg.reach_fixations(first_fix_ind,1)-1,epoch_end))=1;
...
ii_cfg.reach_sel = new_sel;
ii_cfg = ii_updatereachcursel(ii_cfg);


% ordinarily wouldn't put this in history, but because it's used for things
% like drift correction, etc, makes sense here
ii_cfg.reach_history{end+1} = sprintf('ii_selectreachfixationsbytrial - selected epochs %s from chan %s with mode %s - %s',num2str(within_epochs),epoch_chan,sel_mode,datestr(now,30));
